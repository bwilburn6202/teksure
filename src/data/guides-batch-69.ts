...
For switching between apps and windows: Command + Tab shows a row of app icons and lets you switch between running apps by pressing Tab while holding Command. Command + \` (the backtick key, directly above Tab on most keyboards) switches between multiple windows of the same app — useful when you have two Safari windows open, for example. Command + Space opens Spotlight, the system-wide search tool — start typing any file name, app name, or search term to find it instantly.
