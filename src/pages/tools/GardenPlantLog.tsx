      ...
      <div className="max-w-2xl mx-auto px-4 py-8 print:p-4">
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            <Sprout className="h-7 w-7 text-success-foreground" />
            <h1 className="text-3xl font-bold text-foreground">Garden Plant Log</h1>
          </div>
        ...
        {plants.length > 0 && (
          <Card className="mb-4 print:break-inside-avoid">
            <CardContent className="py-3 px-4">
              <p className="text-sm text-muted-foreground mb-2">Garden inventory ({plants.length} plant{plants.length !== 1 ? "s" : ""})</p>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {TYPES.map(t => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md border text-sm font-medium ${form.type === t ? "bg-green-50 border-green-400 text-green-800" : "bg-white border-border text-gray-700"}`}
                  >
                    {TYPE_EMOJI[t]} {t}
                  </button>
                ))}
        ...
        {plants.length === 0 ? (
          <Card className="print:hidden">
            <CardContent className="py-10 text-center text-muted-foreground">
              ...
              <p className="text-base mt-1">Add the first plant in your garden, indoor or out.</p>
            </CardContent>
          </Card>
        ) : (
            ...
            {Object.entries(grouped).map(([type, items]) => (
              <Card key={type} className="print:break-inside-avoid">
                <CardHeader className="pb-2 pt-3 bg-success">
                  <CardTitle className="text-base text-success-foreground">
                      ...
                      return (
                        <div key={p.id} className="py-2 flex items-start justify-between gap-2">
                          ...
                          <div className="flex flex-col gap-1 shrink-0 print:hidden">
                            ...
                            <Button variant="ghost" size="icon" onClick={() => remove(p.id)} className="h-6 w-6 text-danger-foreground hover:text-danger-foreground">
