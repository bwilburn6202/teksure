      ...
      <main className="min-h-screen bg-background"><div className="container max-w-4xl py-12 px-4">
        ...
        <Tabs defaultValue="checkers">
          ...
          <TabsContent value="images">
            <p className="text-base text-muted-foreground mb-4">These tools help detect if an image has been digitally altered, taken out of context, or generated by AI.</p>
            <div className="grid sm:grid-cols-2 gap-3">{IMAGE_VERIFIERS.map(iv => (
              <Card key={iv.name}><CardContent className="p-4"><h3 className="font-semibold text-sm mb-1">{iv.name}</h3><p className="text-sm text-muted-foreground mb-3">{iv.description}</p><a href={iv.url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary font-medium flex items-center gap-1 hover:underline">Open <ExternalLink className="h-3 w-3" /></a></CardContent></Card>
            ))}</div>
          </TabsContent>

          <TabsContent value="bias">
            <p className="text-base text-muted-foreground mb-4">Understanding media bias helps you evaluate sources. These tools rate news outlets on their political leaning and factual accuracy.</p>
            <div className="grid sm:grid-cols-2 gap-3">{BIAS_CHECKERS.map(bc => (
              <Card key={bc.name}><CardContent className="p-4"><h3 className="font-semibold text-sm mb-1">{bc.name}</h3><p className="text-sm text-muted-foreground mb-3">{bc.description}</p><a href={bc.url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary font-medium flex items-center gap-1 hover:underline">Visit <ExternalLink className="h-3 w-3" /></a></CardContent></Card>
            ))}</div>
          </TabsContent>
          ...
          <TabsContent value="redflags">
            <Card><CardContent className="p-6"><h2 className="text-lg font-semibold mb-4">How to Spot Fake News: 8 Red Flags</h2>
              <div className="space-y-3">{RED_FLAGS.map((flag, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-muted/50"><AlertTriangle className="h-5 w-5 text-warn-foreground shrink-0 mt-0.5" /><p className="text-base">{flag}</p></div>
              ))}</div>
              <div className="mt-6 p-4 rounded-lg bg-green-500/10 border border-green-500/30"><h3 className="text-sm font-semibold mb-2 flex items-center gap-2"><CheckCircle className="h-4 w-4 text-success-foreground" /> Quick verification checklist</h3>
                <ol className="space-y-1 text-sm text-muted-foreground list-decimal list-inside">{['Search the headline on Google — are major news outlets reporting it?', 'Check the source — who published this? Are they credible?', 'Read past the headline — does the article body support the headline?', 'Check the date — is this current or old news being reshared?', 'Look for the original source — who said or did this first?'].map((s, i) => <li key={i}>{s}</li>)}</ol>
              </div>
            </CardContent></Card>
          </TabsContent>
          ...
          <TabsContent value="quiz">
            {quizDone ? (
              ...
              <Card><CardContent className="p-6">
                <div className="flex items-center justify-between mb-4"><span className="text-base text-muted-foreground">Headline {quizIdx + 1} of {QUIZ.length}</span><span className="text-base font-medium">{quizScore} correct</span></div>
                ...
                {quizAnswer === null ? (
                  ...
                  <div className="space-y-4">
                    <div className={`rounded-lg p-4 ${quizAnswer === q.isFake ? 'bg-green-500/10 border border-green-500/30' : 'bg-red-500/10 border border-red-500/30'}`}>
                      <div className="flex items-center gap-2 mb-2">{quizAnswer === q.isFake ? <CheckCircle className="h-5 w-5 text-success-foreground" /> : <XCircle className="h-5 w-5 text-danger-foreground" />}<span className="font-semibold">{quizAnswer === q.isFake ? 'Correct!' : 'Not quite.'}</span><Badge variant="outline" className="ml-auto text-xs">{q.isFake ? 'FAKE' : 'REAL'}</Badge></div>
