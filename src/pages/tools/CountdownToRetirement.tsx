...
import {
  ...
  Target, PartyPopper, Sun, Printer,
  type LucideIcon,
} from 'lucide-react';
...
const PHASES: Phase[] = [
  {
    ...
    when: 'About 60 if you plan to retire at 65',
    icon: Hourglass,
  ...
  {
    id: 'one-year',
    title: 'One year before retirement',
    when: 'About 64 if you retire at 65',
    icon: Clock,
  ...
  {
    id: 'six-months',
    title: 'Six months before retirement',
    when: 'About 64 and a half',
    icon: Target,
  ...
  {
    id: 'first-month',
    title: 'First month retired',
    when: 'Day 1 through day 30',
    icon: PartyPopper,
  ...
  {
    id: 'six-after',
    title: 'Six months after retirement',
    when: 'The new normal',
    icon: Sun,
      ...
      <main className="min-h-screen bg-background">
        <section className="bg-gradient-to-br from-amber-50 via-background to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border-b border-border py-10 px-4 print:hidden">
          <div className="container max-w-3xl mx-auto text-center">
            ...
            <p className="text-muted-foreground text-lg max-w-xl mx-auto">
              A year-by-year tech checklist — five phases, from five years out to six months in. Do a few items each phase and the transition stays calm.
            </p>
        ...
        <div className="container max-w-3xl mx-auto px-4 py-8 print:hidden">
          ...
          <Alert className="mb-6 border-amber-300 bg-amber-50 dark:bg-amber-950/30">
            ...
            <AlertDescription className="text-amber-900/80 dark:text-amber-200/80 text-sm">
              Real Medicare will never call out of the blue asking for your Social Security number, bank info, or a new card fee. During the months around your 65th birthday, scammers ramp up. If a caller pressures you, hang up and call 1-800-MEDICARE yourself. Report scams at ftc.gov/complaint.
            </AlertDescription>
          </Alert>
          ...
          <Card className="border-border shadow-sm mb-6">
            <CardContent className="p-5 flex items-center justify-between flex-wrap gap-3">
              ...
              <Button onClick={printIt} variant="outline" size="sm" className="gap-2">
                <Printer className="h-4 w-4" /> Print the checklist
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="border-border bg-muted/30 mb-6">
            <CardContent className="p-5">
              ...
              <div className="grid sm:grid-cols-2 gap-2 text-sm">
                {PHASES.map(p => (
                  <a key={p.id} href={`#${p.id}`} className="flex items-center gap-2 p-2 rounded-md hover:bg-primary/10 hover:text-primary transition-colors">
          ...
          <div className="mt-10 pt-8 border-t border-border">
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              <Link to="/tools/social-security-setup" className="p-3 rounded-lg border border-border hover:border-primary/50 hover:bg-primary/5 transition-all">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Open my Social Security the right way.</p>
              </Link>
              <Link to="/tools/medicare-plan-chooser" className="p-3 rounded-lg border border-border hover:border-primary/50 hover:bg-primary/5 transition-all">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Compare Original, Advantage, and Part D.</p>
              </Link>
              <Link to="/tools/account-after-loss" className="p-3 rounded-lg border border-border hover:border-primary/50 hover:bg-primary/5 transition-all">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Plan what family will need access to.</p>
              </Link>
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-4 flex items-center gap-1">
            <ChevronRight className="h-3 w-3" /> Quick Tip: stick the printed checklist on the fridge and cross off one item a week. Five years of small steps beats one frantic month.
          </p>
        </div>
