      ...
      <div className="max-w-xl mx-auto px-4 py-8">
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-3xl font-bold text-foreground">Hourly to Annual Salary</h1>
          </div>
          <p className="text-muted-foreground text-lg">Quickly convert an hourly wage to weekly, monthly, and yearly pay.</p>
        </div>
        <Card>
          <CardContent className="py-6 px-4 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-base font-medium text-foreground">Hourly rate ($)</label>
            ...
            <div className="flex flex-wrap gap-1">
              {[10, 20, 25, 30, 35, 40].map(n => (
                <button key={n} onClick={() => setHoursPerWeek(String(n))} className="text-xs px-2 py-1 rounded-full bg-success border border-success-foreground/25 text-success-foreground hover:bg-success">{n} hrs</button>
              ))}
            </div>
            {valid && (
              <div className="space-y-2">
                <div className="flex justify-between p-3 bg-success rounded"><span className="text-base">Weekly</span><span className="text-xl font-bold text-success-foreground">{fmt(weekly)}</span></div>
                <div className="flex justify-between p-3 bg-success rounded"><span className="text-base">Monthly</span><span className="text-xl font-bold text-success-foreground">{fmt(monthly)}</span></div>
                <div className="flex justify-between p-3 bg-success rounded"><span className="text-base font-semibold">Yearly</span><span className="text-2xl font-bold text-success-foreground">{fmt(annual)}</span></div>
              </div>
            )}
          </CardContent>
        </Card>
