      ...
      <div className="max-w-2xl mx-auto px-4 py-8 print:p-4">
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            <Wine className="h-7 w-7 text-danger-foreground" />
            <h1 className="text-3xl font-bold text-foreground">Alcohol Moderation Tracker</h1>
          </div>
          <p className="text-muted-foreground text-lg">For older adults curious about their patterns. No judgment. Patterns are easier to change once you can see them.</p>
        </div>

        {recent.length >= 5 && (
          <div className="grid grid-cols-3 gap-3 mb-4">
            <Card><CardContent className="py-3 px-3 text-center"><p className="text-sm text-muted-foreground">14-day total</p><p className="text-2xl font-bold text-danger-foreground">{totalRecent.toFixed(0)}</p></CardContent></Card>
            <Card><CardContent className="py-3 px-3 text-center"><p className="text-sm text-muted-foreground">Daily avg</p><p className="text-2xl font-bold text-danger-foreground">{avgDaily.toFixed(1)}</p></CardContent></Card>
            <Card><CardContent className="py-3 px-3 text-center"><p className="text-sm text-muted-foreground">Dry days</p><p className="text-2xl font-bold text-success-foreground">{dryDays}</p></CardContent></Card>
          </div>
        )}
        ...
        <Card className="mb-4 print:hidden">
          ...
          <CardContent className="space-y-2">
            ...
            <div>
              <label className="text-base font-medium text-foreground">Number of drinks today (a drink = 12 oz beer, 5 oz wine, or 1.5 oz spirits)</label>
            ...
            <div>
              <label className="text-base font-medium text-foreground">Mostly which?</label>
              <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value as DrinkType }))} className="w-full border border-border rounded-md px-3 py-2 text-base bg-card mt-1">
                {TYPES.map(t => <option key={t}>{t}</option>)}
              </select>
            </div>
            <div>
              <label className="text-base font-medium text-foreground">Urge level today (1 = no urge, 5 = strong urge)</label>
                ...
                {[...entries].reverse().map(e => (
                  <div key={e.id} className="py-2 text-sm flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 flex-wrap flex-1 min-w-0">
                      <span className="text-muted-foreground text-sm">{e.date}</span>
                      <span className="font-bold">{e.drinks}</span>
                      <Badge variant="secondary" className="text-xs">{e.type}</Badge>
                      <span className="text-sm text-muted-foreground">Urge: {e.urgeLevel}/5</span>
                      {e.note && <p className="text-sm text-muted-foreground italic w-full">{e.note}</p>}
                    </div>
