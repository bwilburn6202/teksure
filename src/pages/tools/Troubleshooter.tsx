    ...
    <>
      ...
      <main className="container max-w-2xl py-12 min-h-[70vh]">
        ...
        <div className="flex items-start gap-3 mb-8">
          <HelpCircle className="h-8 w-8 text-secondary shrink-0 mt-0.5" />
          <div>
          ...
          {!category && (
            <motion.div key="categories" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                ...
                {Object.entries(trees).map(([key, t]) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-4 p-4 rounded-xl border border-border bg-card hover:border-secondary hover:shadow-md transition-all text-left group"
                    >
                      <div className="h-11 w-11 rounded-xl bg-muted flex items-center justify-center shrink-0">
                        <Icon className={`h-5 w-5 ${t.color}`} />
                      </div>
                      <span className="font-medium group-hover:text-secondary transition-colors">{t.label}</span>
              ...
              <div className="mt-8 p-4 rounded-xl bg-muted/50 border border-border text-sm text-muted-foreground flex items-start gap-3">
                ...
                <span>This tool helps you fix problems yourself. For complex or urgent issues, <Link to="/get-help" className="text-secondary hover:underline font-medium">get help from a real technician →</Link></span>
              </div>
            </motion.div>
          )}
