      ...
      <div className="max-w-xl mx-auto px-4 py-8">
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-3xl font-bold text-foreground">Cooking Measurement Converter</h1>
          </div>
          <p className="text-muted-foreground text-lg">Type a number and see how it converts across the common kitchen units.</p>
        </div>

        <Card className="mb-4">
          <CardContent className="py-4 px-4">
            <label className="text-base font-medium text-foreground">Amount</label>
            ...
            <div className="divide-y">
              {CONVERSIONS.map((c, i) => (
                <div key={i} className="py-2 flex items-center justify-between text-base">
                  <span className="text-foreground">{!isNaN(n) ? n : "?"} {c.from}{!isNaN(n) && n !== 1 ? "s" : ""}</span>
