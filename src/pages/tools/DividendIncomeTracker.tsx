      ...
      <div className="max-w-2xl mx-auto px-4 py-8 print:p-4">
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-3xl font-bold text-foreground">Dividend Income Tracker</h1>
          </div>
        ...
        <Card className="mb-4 print:break-inside-avoid">
          <CardContent className="py-3 px-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="text-right">
              <p className="text-sm text-muted-foreground">Total dividends {taxYear}</p>
                ...
                {symbolTotals.map(s => (
                  <div key={s.symbol} className="flex items-center justify-between py-1 text-sm">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-sm text-muted-foreground">{s.count} payment{s.count !== 1 ? "s" : ""}</span>
                      <span className="font-bold text-success-foreground">{formatMoney(s.total)}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sorted.length === 0 ? (
          <Card className="print:hidden">
            <CardContent className="py-10 text-center text-muted-foreground">
              ...
              <p className="text-base mt-1">Open your brokerage statement and log this month's payments.</p>
            </CardContent>
          </Card>
        ) : (
          <Card>
                ...
                {sorted.map(d => (
                  <div key={d.id} className="py-2 flex items-center justify-between gap-2 text-sm print:break-inside-avoid">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <span className="text-xs text-muted-foreground w-20 shrink-0">{formatDate(d.date)}</span>
                      <Badge variant="secondary" className="text-xs font-mono">{d.symbol}</Badge>
                      {d.notes && <span className="text-xs text-muted-foreground italic truncate">{d.notes}</span>}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="font-semibold text-success-foreground">{formatMoney(d.amount)}</span>
                      <Button variant="ghost" size="icon" onClick={() => remove(d.id)} className="h-6 w-6 text-danger-foreground hover:text-danger-foreground print:hidden">
