      ...
      <div className="max-w-3xl mx-auto px-4 py-8 print:p-0 print:max-w-full">
        <div className="mb-6 print:hidden">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-3xl font-bold text-foreground">Mail Label Maker</h1>
          </div>
          <p className="text-muted-foreground text-lg">Type each address. When you have your set, click Print — addresses lay out in a clean grid on letter-size paper.</p>
        </div>
        ...
        {addrs.length > 0 && (
          <div className="mb-3 flex items-center justify-between print:hidden">
            <p className="text-base text-muted-foreground">{addrs.length} label{addrs.length !== 1 ? "s" : ""}</p>
        ...
        <div className="grid grid-cols-2 gap-2 print:gap-0">
          {addrs.map(a => (
            <div key={a.id} className="border border-border rounded p-3 bg-card text-sm relative print:border-black print:rounded-none print:break-inside-avoid">
              <p className="font-semibold">{a.name}</p>
              <p>{a.line1}</p>
              {a.line2 && <p>{a.line2}</p>}
              <p>{a.cityStateZip}</p>
              <Button variant="ghost" size="icon" onClick={() => remove(a.id)} className="absolute top-1 right-1 h-6 w-6 text-danger-foreground hover:text-danger-foreground print:hidden">
        ...
        {addrs.length === 0 && (
          <Card className="print:hidden">
            <CardContent className="py-10 text-center text-muted-foreground">
              ...
              <p className="text-base mt-1">Add an address above to get started.</p>
            </CardContent>
          </Card>
        )}
      </div>
