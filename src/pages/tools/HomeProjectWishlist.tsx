      ...
      <div className="max-w-2xl mx-auto px-4 py-8 print:p-4">
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            <Wrench className="h-7 w-7 text-warn-foreground" />
            <h1 className="text-3xl font-bold text-foreground">Home Project Wishlist</h1>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <Card>
              <CardContent className="py-3 px-3 text-center">
                <p className="text-sm text-muted-foreground">To do or in progress</p>
                <p className="text-xl font-bold text-warn-foreground">{todoCount}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="py-3 px-3 text-center">
                <p className="text-sm text-muted-foreground">Done</p>
                <p className="text-xl font-bold text-success-foreground">{doneCount}</p>
              </CardContent>
            </Card>
          </div>
        )}
        ...
        <Card className="mb-6 print:hidden">
          ...
          <CardContent className="space-y-3">
              ...
              <div className="flex gap-2 mt-1">
                {(["now", "soon", "someday"] as Priority[]).map(p => (
                  <button
                    ...
                    className={`flex-1 px-3 py-2 rounded-md border text-sm font-medium ${form.priority === p ? "bg-orange-50 border-warn-foreground/25 text-orange-800" : "bg-white border-border text-gray-700"}`}
                  >
                    {PRIORITIES[p].label}
                  </button>
                ))}
            ...
            <div>
              ...
              <select value={form.helper} onChange={e => setForm(f => ({ ...f, helper: e.target.value as Helper }))} className="mt-1 w-full border border-border rounded-md px-3 py-2 text-base bg-card">
        ...
        {sorted.length === 0 ? (
          <Card className="print:hidden">
            <CardContent className="py-10 text-center text-muted-foreground">
              ...
              <p className="text-base mt-1">Add the first thing on your "to do around the house" list.</p>
            </CardContent>
          </Card>
        ) : (
                ...
                <CardContent className="py-3 px-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex-1 min-w-0">
                      <p className={`font-semibold text-foreground text-base ${p.status === "done" ? "line-through text-gray-400" : ""}`}>{p.name}</p>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">Helper: {p.helper}</p>
                  ...
                  <div className="flex flex-wrap gap-1 print:hidden">
                    {(["todo", "doing", "done"] as Status[]).map(s => (
                      <button
                        ...
                        className={`text-xs px-2 py-1 rounded border ${p.status === s ? "bg-orange-50 border-warn-foreground/25 text-warn-foreground font-medium" : "bg-white border-border text-muted-foreground hover:bg-gray-50"}`}
                      >
