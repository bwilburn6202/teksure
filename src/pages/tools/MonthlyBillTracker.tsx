        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-3 mb-6 print:grid-cols-3">
            <div className="bg-card rounded-xl border border-border p-4 text-center shadow-sm">
              ...
              <p className="text-base text-muted-foreground mt-1">Monthly total</p>
            </div>
            <div className="bg-card rounded-xl border border-border p-4 text-center shadow-sm">
              <p className="text-2xl font-bold text-foreground ">{bills.length}</p>
              <p className="text-base text-muted-foreground mt-1">Bills tracked</p>
            </div>
            <div className="bg-card rounded-xl border border-border p-4 text-center shadow-sm">
              <p className="text-2xl font-bold text-info-foreground ">{autopayCount}</p>
              <p className="text-base text-muted-foreground mt-1">On autopay</p>
            </div>
          </div>

          {/* Missing amounts notice */}
          {missingAmount.length > 0 && (
            <div className="flex gap-2 bg-warn border border-warn-foreground/25 rounded-xl p-4 mb-4 print:hidden">
          ...
          <div className="bg-card rounded-2xl border border-border overflow-hidden shadow-sm mb-4">
            <div className="px-5 py-3 bg-muted border-b border-border flex justify-between items-center print:hidden">
              <span className="font-medium text-foreground text-base">Your bills — sorted by due date</span>
            </div>
                ...
                <div key={bill.id} className="px-4 py-3">
                  ...
                  <div className="hidden print:flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-base text-muted-foreground ml-2">({bill.category})</span>
                    </div>
                  ...
                  <div className="print:hidden">
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <input
                            ...
                            className="font-medium text-foreground bg-transparent border-b border-transparent hover:border-border focus:border-emerald-400 outline-none flex-1 min-w-0"
                          />
                          {dueSoon(bill.dueDay) && (
                            <span className="text-xs bg-danger text-danger-foreground px-2 py-0.5 rounded-full font-medium">
                              Due soon
                            </span>
                          )}
                          {bill.autopay && (
                            <span className="text-xs bg-info text-info-foreground px-2 py-0.5 rounded-full">
                              Autopay
                            </span>
                          )}
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <div className="flex items-center gap-1">
                            ...
                            <input
                              ...
                              className="text-sm text-foreground bg-muted rounded-lg px-2 py-1 w-full outline-none border border-border focus:border-emerald-400"
                            />
                          </div>
                          <div className="flex items-center gap-1">
                            <span className="text-sm text-muted-foreground whitespace-nowrap">Due day</span>
                            <input
                              type="number"
                              min="1"
                              max="31"
                              ...
                              className="text-sm text-foreground bg-muted rounded-lg px-2 py-1 w-16 outline-none border border-border focus:border-emerald-400"
                            />
                          </div>
                          <div>
                            <select
                              ...
                              className="text-sm text-foreground bg-muted rounded-lg px-2 py-1 w-full outline-none border border-border "
                            >
                              {CATEGORIES.map(c => (
                      ...
                      <button
                        ...
                        className="flex-shrink-0 text-muted-foreground hover:text-danger-foreground transition-colors mt-1"
            ...
            {filledBills.length > 0 && (
              <div className="px-5 py-3 bg-success border-t border-emerald-100 dark:border-emerald-900 flex justify-between items-center">
                <span className="font-semibold text-success-foreground ">Monthly Total</span>
                <span className="font-bold text-lg text-success-foreground ">${total.toFixed(2)}</span>
              </div>
            )}
          </div>
          ...
          <div className="print:hidden mb-4">
            {!showForm ? (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 px-5 rounded-xl border-2 border-dashed border-border text-muted-foreground hover:border-emerald-400 hover:text-success-foreground transition-all"
              >
              ...
              <div className="bg-card rounded-2xl border border-border p-5 shadow-sm">
                <h3 className="font-semibold text-foreground mb-4">Add a new bill</h3>
                <div className="space-y-3">
                  <div>
                    <label className="block text-base text-muted-foreground mb-1">Bill name</label>
                    <input
                      type="text"
                      placeholder="e.g. Netflix, Water bill, Car insurance"
                      value={newName}
                      onChange={e => setNewName(e.target.value)}
                      className="w-full rounded-lg px-3 py-2 border border-border bg-muted text-foreground outline-none focus:border-emerald-400"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-base text-muted-foreground mb-1">Amount ($)</label>
                      <input
                        type="number"
                        placeholder="0.00"
                        value={newAmount}
                    ...
                    <div>
                      <label className="block text-base text-muted-foreground mb-1">Due day of month</label>
                      <input
                        type="number"
                        min="1"
                        max="31"
                  ...
                  <div>
                    <label className="block text-base text-muted-foreground mb-1">Category</label>
                    <select
                      value={newCategory}
          ...
          {/* Privacy note */}
          <div className="mt-6 flex gap-3 bg-info border border-info-foreground/25 rounded-xl p-4 print:hidden">
